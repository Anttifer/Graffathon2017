...
GUI::GUI(MainWindow& window, Layering& layering) :
	...
	position_4d_default_             (0.0f, 0.0f, 0.0f, 2.5f),
	position_3d_default_             (1.0f, 1.0f, 2.0f),
	fov_4d_default_                  (0.5f * M_PI),
...
void GUI::draw_4d_settings(void)
{
	ImGui::Text("4D position:"); ImGui::SameLine(120);
	ImGui::PushItemWidth(-5.0f);
	ImGui::DragFloat4("##4D position", position_4d_->data(), 0.01f);
	ImGui::PopItemWidth();

	ImGui::Text("3D position:"); ImGui::SameLine(120);
	ImGui::PushItemWidth(-5.0f);
	ImGui::DragFloat3("##3D position", position_3d_->data(), 0.01f);
	ImGui::PopItemWidth();

	float fov = *fov_4d_ / M_PI * 180.0f;
	ImGui::Text("FOV:"); ImGui::SameLine(120);
	ImGui::PushItemWidth(-5.0f);
	if (ImGui::DragFloat("##4D fov", &fov, 0.5f))
		*fov_4d_ = fov / 180.0f * M_PI;
	ImGui::PopItemWidth();
}
